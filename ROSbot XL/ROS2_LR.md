...
On some devices, like the Husarion ROSbotXL, it is necessary to have the same *.pth* file also in the directory */usr/local/lib/python3.XX/dist-packages/* adding the following two lines:
``` bash
/opt/ros/<ROS_DISTRO>/local/lib/python3.XX/dist-packages
/opt/ros/<ROS_DISTRO>/lib/python3.XX/site-packages
```
